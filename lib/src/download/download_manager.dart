...
class FDownloadManager extends ChangeNotifier {
  ...
  Future<void> initialize([FDownloadConfig config = const FDownloadConfig()]) async {
    ...
    _isInitialized = true;
    _subscription = _platform.queue.listen(_onQueue);
    _items = await _platform.list();
  ...
  Future<void> pause(String id) => _platform.setPaused(id: id, paused: true);

  /// Resumes a paused or failed download from where it stopped.
  Future<void> resume(String id) => _platform.setPaused(id: id, paused: false);

  /// Stops every download.
  Future<void> pauseAll() => _platform.setAllPaused(paused: true);

  Future<void> resumeAll() => _platform.setAllPaused(paused: false);

  /// Deletes a download and its bytes.
  Future<void> remove(String id) => _platform.remove(id);

  /// Deletes everything, including partial transfers.
  Future<void> removeAll() => _platform.removeAll();
  ...
  void _onQueue(List<FDownloadItem> items) {
    if (_isDisposed) return;
    // Newest first: a downloads screen is a list of what the user just asked for.
