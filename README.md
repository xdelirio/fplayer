...
| `showQualityBitrate` | the kbps beside each rung in that ladder. Off by default; `true` adds it |
...
The quality menu shows one row per rung: manifests routinely carry several renditions of the same size, and a list with `1080p` three times asks a question nobody can answer. Each size keeps its best rendition; `tracks.video` still holds them all. Each row is just the rung. `FUiConfig(showQualityBitrate: true)` puts the bitrate beside it, which is the number that actually separates two renditions of the same size — useful to someone watching their data, noise to everyone else.
