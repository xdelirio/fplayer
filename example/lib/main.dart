...
const _demos = <_Demo>[
  _Demo(
    ...
    FPlayerSource.network(
      ...
      title: 'Angel One',
      subtitle: 'Multi-audio y subtítulos embebidos',
  ...
  _Demo(
    ...
    FPlayerSource.network(
      ...
      subtitle: 'Escalera de calidades 224p → 1680×750',
    ),
  ),
  _Demo(
    ...
    FPlayerSource.network(
      ...
      title: 'BipBop',
      subtitle: 'Patrón de prueba, 30 minutos',
    ),
  ),
  _Demo(
    'MP4 local + subtítulo externo + storyboard',
    FPlayerSource.network(
      '$_host/sample.mp4',
      title: 'Verificación de headers',
      subtitle: 'El VTT solo recibe sus propios headers',
      headers: {
  ...
  _Demo(
    ...
    queue: [
      ...
      FPlayerSource.network(
        ...
        type: FSourceType.hls,
        title: 'BipBop',
        subtitle: 'Episodio 3 de 3',
      ),
    ],
  ),
  _Demo(
    'URL inválida (prueba de errores)',
    FPlayerSource.network(
  ...
  Widget build(BuildContext context) {
    ...
    final rows = <String, String>{
      ...
      'video': '${tracks.video.length} · '
          '${tracks.isVideoAuto ? "auto" : "fijo"} '
          '${tracks.activeVideo?.qualityLabel ?? "—"}',
      'cues': value.cues.isEmpty ? '—' : value.cues.map((c) => c.text).join(' | '),
      'cola': value.hasPlaylist
          ? '${value.currentIndex + 1}/${value.playlist.length} · siguiente '
              '${value.nextInPlaylist?.title ?? "—"}'
          : '—',
      'red': value.isOnline ? 'en línea' : 'sin conexión',
      'pip': 'soportado ${value.isPipSupported} · activo ${value.isPipActive}',
      'sesión': 'vistos ${metrics?.watchedTime ?? "—"} · '
